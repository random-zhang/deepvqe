...
class DeepVQE_AEC(nn.Module):
    """DeepVQE with AlignBlock enabled for AEC.
    Accepts mic mix and far-end reference, aligns far-end, then concatenates
    with mic features before encoding.
    """
    def __init__(self, align_hidden=2, align_delay=100):
        super().__init__()
        self.fe = FE()
        self.align = AlignBlock(in_channels=2, hidden_channels=align_hidden, delay=align_delay)
        # First encoder now takes mic(2) + aligned far(align_hidden)
        self.enblock1 = EncoderBlock(2 + align_hidden, 64)
        self.enblock2 = EncoderBlock(64, 128)
        self.enblock3 = EncoderBlock(128, 128)
        self.enblock4 = EncoderBlock(128, 128)
        self.enblock5 = EncoderBlock(128, 128)
    ...
    def forward(self, x_mic, x_far):
        """x_mic, x_far: (B,F,T,2)"""
        mic_feat = self.fe(x_mic)  # (B,2,T,F)
        far_feat = self.fe(x_far)  # (B,2,T,F)
        far_aligned = self.align(mic_feat, far_feat)  # (B,align_hidden,T,F)

        en_x0 = torch.cat([mic_feat, far_aligned], dim=1)
        ...
        x_enh = self.ccm(de_x1, x_mic)
        return x_enh



if __name__ == "__main__":
    # AEC model with AlignBlock
    model = DeepVQE_AEC().eval()
    x_mic = torch.randn(1, 257, 63, 2)
    x_far = torch.randn(1, 257, 63, 2)
    y = model(x_mic, x_far)

    # Simple causality check: change future mic frames while keeping far-end fixed
    a_mic = torch.randn(1, 257, 100, 2)
    b_mic = torch.randn(1, 257, 100, 2)
    c_mic = torch.randn(1, 257, 100, 2)
    far = torch.randn(1, 257, 200, 2)
