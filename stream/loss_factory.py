...
class HybridLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        pred_stft = torch.stft(y_pred, self.n_fft, self.hop_len, self.win_len, self.window.to(device), return_complex=True, center=True)
        true_stft = torch.stft(y_true, self.n_fft, self.hop_len, self.win_len, self.window.to(device), return_complex=True, center=True)
        ...
        # 打印每种loss的值
        #print(f"real_loss: {real_loss.item():.6f}, imag_loss: {imag_loss.item():.6f}, mag_loss: {mag_loss.item():.6f}, sisnr: {sisnr.item():.6f}")
...
class STFTLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x = torch.stft(x, self.n_fft, self.hop_len, self.win_len, self.window.to(x.device), return_complex=True, center=True)
        y = torch.stft(y, self.n_fft, self.hop_len, self.win_len, self.window.to(x.device), return_complex=True, center=True)
